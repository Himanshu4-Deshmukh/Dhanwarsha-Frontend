...
//       if (!silent) {
//         toast.error("Unable to load lottery data");
//       }
//     } finally {
...
//   const walletCanCover = balance >= (todayRound?.ticketPrice ?? config?.ticketPrice ?? 0);
...
//           <InfoCard icon={Coins} label="Wallet" value={`${balance} coins`} />
//           <InfoCard icon={Ticket} label="Ticket price" value={`${todayRound?.ticketPrice ?? config?.ticketPrice ?? 100} Rs`} />
...
//           </span>
//           <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
//             {todayRound?.ticketDigits ?? 6}-digit ticket
//           </span>
//           <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
//             {todayRound?.totalPurchases ?? 0} tickets sold
//           </span>
//         </div>
//       </div>
...
//               100 numbers are released every day
//             </h2>
//           </div>
...
//             {todayRound?.status === "OPEN" ? "Purchase open" : "Purchasing closed"}
//           </div>
//         </div>

//         <div className="grid grid-cols-5 gap-2 sm:grid-cols-10">
//           {numbers.map((entry) => {
//             const isSelected = selectedNumber === entry.number;
...
//                 className={`relative rounded-2xl border px-2 py-3 text-left transition ${
//                   isSelected
//                     ? "border-primary bg-primary/15 text-white"
//                     : hot
//                       ? "border-orange-500/20 bg-orange-500/10 text-orange-200"
//                       : "border-white/10 bg-white/5 text-white/70 hover:border-white/20 hover:bg-white/10"
//                 } ${!canPurchase ? "cursor-not-allowed opacity-45" : ""}`}
//               >
...
//                 {selectedTicket?.number || "Pick a ticket"}
//               </h3>
//               <p className="mt-1 text-sm text-white/50">
//                 {selectedTicket
//                   ? `Ticket selected for ${todayRound?.ticketPrice ?? 100} Rs`
//                   : "Select a 6-digit number above to continue"}
//               </p>
//             </div>

//             <div className="flex flex-wrap gap-2 text-xs text-white/60">
//               <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
//                 {todayRound?.totalPurchases ?? 0} total purchases
//               </span>
//               <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
//                 {todayRound?.totalSales ?? 0} Rs sales
//               </span>
//             </div>
//           </div>
...
//             <button
//               type="button"
//               disabled={!selectedNumber || !canPurchase || buying || !walletCanCover}
//               onClick={handleBuy}
//               className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-primary to-emerald-500 px-4 py-3 text-sm font-bold text-black transition disabled:cursor-not-allowed disabled:opacity-50"
//             >
//               {buying ? (
//                 <>
...
//               ) : (
//                 <>
//                   <Coins className="h-4 w-4" /> Buy ticket
//                 </>
//               )}
//             </button>
...
//               {walletCanCover
//                 ? "Wallet balance is enough for this ticket."
//                 : "Add wallet balance before buying a ticket."}
//             </p>
//           </div>
//         </div>
//       </div>
...
//                       rightTone={item.status === "WON" ? "success" : item.status === "LOST" ? "danger" : "normal"}
//                     />
//                   </>
//                 )}
//               />
...
//                       rightValue={`+${item.payout || todayRound?.winAmount || config?.winAmount || 5000}`}
//                       rightTone="success"
//                     />
//                   </>
//                 )}
...
//                       rightTone={item.status === "RESULT_DECLARED" ? "success" : "normal"}
//                     />
//                   </>
//                 )}
//               />
...
//         <p className="text-xs uppercase tracking-[0.3em] text-white/35">{title}</p>
      ...
      <div className="rounded-[28px] border border-white/10 bg-[hsl(220,20%,8%)] p-4">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-lg font-bold text-white">
              100 numbers released daily
            </h2>
          </div>
          <div className="flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/55">
            <Sparkles className="h-3.5 w-3.5 text-primary" />
            {canPurchase ? "Purchase open" : "Purchasing closed"}
          </div>
        </div>

        {/* 3-per-row ticket grid */}
        ...
        <div className="mt-5 rounded-3xl border border-white/10 bg-black/25 p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-white/50">
                {selectedTicket
                  ? `Ticket selected for ${ticketPrice} Rs`
                  : "Select a 6-digit number above to continue"}
              </p>
            </div>
            <div className="flex flex-wrap gap-2 text-xs text-white/60">
