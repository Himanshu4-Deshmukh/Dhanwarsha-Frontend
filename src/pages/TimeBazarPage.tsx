...
const SUNDAY_CLOSED_GAMES = new Set([
  "time-bazar",
  "rajdhani-day",
]);
const isClosedToday = (gameKey: string): boolean => {
  const nowIST = new Date(Date.now() + 5.5 * 60 * 60 * 1000);
  const dayOfWeek = nowIST.getUTCDay(); // 0 = Sunday
  return dayOfWeek === 0 && SUNDAY_CLOSED_GAMES.has(gameKey);
};
const isResultFromToday = (
  fetchedAt?: string,
  startTime?: string,
  ...
  if (isNaN(fetched.getTime()) || isNaN(gameStart.getTime())) return false;
  ...
  return toISTDate(fetched) === toISTDate(gameStart);
};

const maskLiveNumber = (rawNumber?: string | null): string => {
  if (!rawNumber) return "***-**-***";
  if (/^\d{3}-\d{2}-\d{3}$/.test(rawNumber)) return rawNumber; // full
  if (/^\d{3}-\d$/.test(rawNumber)) return `${rawNumber}*-***`; // open only
  return "***-**-***";
};
...
const TimeBazarPage = () => {
  ...
  const loadLiveDraws = useCallback(async (silent = false) => {
    if (!silent) setLiveDrawsLoading(true);
    try {
      const draws = await api.getLiveDraws();
      setLiveDraws(draws ?? []);
    } catch (error) {
      ...
      if (!silent) setLiveDrawsLoading(false);
    }
  }, []);
  ...
  const sortedLiveDraws = useMemo(() => {
    if (!liveDraws.length) return [];

    const closedToday = (gameKey: string) => isClosedToday(gameKey);

    return [...liveDraws].sort((a, b) => {
      // Sunday-closed games sink to the bottom
      const aClosedToday = closedToday(a.gameKey);
      const bClosedToday = closedToday(b.gameKey);
  ...
  const openPlayModal = (gameKey: string) => {
    if (isClosedToday(gameKey)) {
      const draw = sortedLiveDraws.find((d) => d.gameKey === gameKey);
      setInfoModalDraw(draw ?? null);
      setInfoModalOpen(true);
      return;
    }
  ...
  const selectedDraw = useMemo(() => {
    if (!sortedLiveDraws.length) return null;
    if (selectedGameKey) {
      return (
        sortedLiveDraws.find((draw) => draw.gameKey === selectedGameKey) ??
        sortedLiveDraws[0]
      );
    }
    return sortedLiveDraws[0];
  }, [sortedLiveDraws, selectedGameKey]);
  ...
  const numberRange = useMemo(() => {
    if (selectedDraw?.numberRange) return selectedDraw.numberRange;
    return { start: 0, end: 99 };
  }, [selectedDraw?.numberRange?.start, selectedDraw?.numberRange?.end]);
  ...
  return (
    <>
      <div className="space-y-4 px-4 pb-28 pt-6">
        ...
        {/* ── Draw cards (sorted correctly) ── */}
        {sortedLiveDraws.map((draw) => {
          const result = latestResults.find((r) => r.gameKey === draw.gameKey);
          const closedForToday = isClosedToday(draw.gameKey);

          const isLiveStatus = ["OPEN", "LIVE", "RUNNING"].includes(
            draw.status?.toUpperCase(),
          );

          // Fetch timestamps from either source
          const latestFetchedAt = result?.fetchedAt ?? draw.latestResult?.fetchedAt;
          const latestRaw = result?.rawNumber ?? draw.latestResult?.rawNumber ?? null;

          // Only use a result if it was fetched on the same IST calendar day as this draw's startTime
          const resultIsToday = isResultFromToday(latestFetchedAt, draw.startTime);

          // ── Determine display number ──────────────────────────────────────
          let rawNumber: string;

          if (closedForToday) {
            // Game doesn't run today — never show a stale number
            rawNumber = "***-**-***";
          } else if (isLiveStatus) {
            // Game is currently live — only show a today result, masked appropriately
            rawNumber = resultIsToday ? maskLiveNumber(latestRaw) : "***-**-***";
          } else {
            // CLOSED / RESULT_DECLARED — show today's result or placeholder
            rawNumber = resultIsToday ? (latestRaw ?? "***-**-***") : "***-**-***";
          }
          ...
          } else if (isLiveStatus && resultIsToday && /^\d{3}-\d$/.test(latestRaw ?? "")) {
            status = "RUNNING FOR CLOSE";
          } else {
            status = getDrawDisplayStatus(draw.status);
          }

          const closed = closedForToday || isDrawClosed(draw.status);
          const upcoming = !closedForToday && isDrawUpcoming(draw.status);
          const canBet = !closedForToday && isDrawOpenForBetting(draw.status);
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  {(() => {
                    // ── Closed for today (Sunday or no data) ──────────────
                    if (closedForToday) {
                      return (
                        <>
                          <div className="flex items-center gap-2">
          ...
          latestResults.map((result) => (
            ...
            >
              ...
              <div className="mt-4 flex gap-3 border-t border-white/10 pt-3 text-xs text-white/50">
                ...
                <span>
                  CLOSE BIDS:{" "}
                  <span className="text-white/80">{result.closeTime ?? "--"}</span>
                </span>
              </div>
            </div>
          ))}
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h2 className="text-2xl font-bold text-white">{modalGameName}</h2>
                {modalStage === "betting" && (
                  <p className="text-xs text-white/50">{selectedGameTypeLabel}</p>
                )}
              </div>
              <button
                type="button"
              ...
              <div className="mt-6 space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {GAME_TYPES.map((type) => {
                    ...
                    // Only disable open if we have a verified today result with openNumber
                    const fetchedAt =
                      selectedResult?.fetchedAt ??
                      selectedDraw?.latestResult?.fetchedAt;
                    const resultToday = isResultFromToday(
                      fetchedAt,
                      selectedDraw?.startTime,
                    );
                    const isOpenDeclared =
                      type.key === "open" &&
                      resultToday &&
                      !!selectedResult?.openNumber?.trim();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
          ...
          <div className="relative w-full max-w-sm overflow-hidden rounded-3xl border border-white/10 bg-[hsl(220,20%,10%)] shadow-[0_30px_80px_rgba(0,0,0,0.8)]">
            ...
            <div className="px-6 pt-8 pb-5 text-center">
              ...
              <div
                className={`mt-3 inline-flex items-center gap-1.5 rounded-full px-3 py-1 ${isClosedToday(infoModalDraw?.gameKey ?? "")
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${isClosedToday(infoModalDraw?.gameKey ?? "")
                      ? "bg-white/40"
                      : isDrawUpcoming(infoModalDraw?.status)
                        ? "bg-amber-400"
                        : "bg-red-400"
                ...
                />
                <p className="text-xs font-semibold">
                  {isClosedToday(infoModalDraw?.gameKey ?? "")
                    ? "Closed for Today"
                    : getDrawDisplayStatus(infoModalDraw?.status)}
                </p>
              </div>
            ...
            <div className="mx-4 my-5 overflow-hidden rounded-2xl border border-white/10">
              {(() => {
                const infoResult = latestResults.find(
                  (r) => r.gameKey === infoModalDraw?.gameKey,
                );
                const openBidTime =
                  infoResult?.openTime ??
                ...
                const closeBidTime =
                  infoResult?.closeTime ??
                  formatClockTime(infoModalDraw?.endTime);
                const openResultTime = addMinutes(openBidTime, 10);
                const closeResultTime = addMinutes(closeBidTime, 10);

                // Only show result if it's from today
                const infoFetchedAt =
                  infoResult?.fetchedAt ??
                  infoModalDraw?.latestResult?.fetchedAt;
                const infoResultToday = isResultFromToday(
                  infoFetchedAt,
                  infoModalDraw?.startTime,
                );
                const resultNumber = infoResultToday
                  ? (infoResult?.rawNumber ??
                    infoModalDraw?.latestResult?.rawNumber ??
                    null)
                  : null;
